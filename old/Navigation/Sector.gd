...
class Sector:
	...
	var cells_positions_index: Dictionary = {}
	...
	func clone():
		...
		new_sector.cells_positions_index = self.cells_positions_index
		return new_sector
	...
	func add_cell(position, cost, flow=Vector2()):
		...
		cells_positions_index[position] = cells.size() - 1
	...
	func get_nearest_portal(position: Vector2):
		...
		for portal in self.portals:
			if position.distance_to(portal.position) < lowest_distance:
				lowest_distance = position.distance_to(portal.position)
				lowest_index = index
			index += 1
		return lowest_index
	...
	func find_cell(position) -> int:
		return cells_positions_index.get(position, -1)
