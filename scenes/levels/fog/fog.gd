...
func _ready():
	light_offset = Vector2(float(LightTexture.get_width()/2.0), float(LightTexture.get_height()/2.0))
	light_image = LightTexture.get_image()
...
func update_fog(new_grid_position, unit_vision_range: int):
	...
	img.resize(floor((unit_vision_range * 2)/GRID_SIZE),  floor((unit_vision_range * 2)/GRID_SIZE))
	light_offset = Vector2(img.get_width() / 2.0, img.get_height() / 2.0)
